...
export function NativeAggregatedChart() {
  ...
  const chartDataForMetrics = selectedMetrics.map(metricKey => {
    const metric = getMetric(metricKey);
    if (!metric) return null;

    const chartPoints = chartData.map(item => ({
      x: item.period,
      y: item[metricKey as keyof AggregatedData] as number
    }));
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Aggregated Metrics (Native Charts)</CardTitle>
      ...
      <CardContent>
        {chartData.length === 0 ? (
          ...
          <div className="space-y-6">
            {chartDataForMetrics.map((chartInfo, index) => {
              if (!chartInfo) return null;
              
              return (
                <NativeChart
                  key={chartInfo.metric.key}
                  data={chartInfo.data}
                  width={800}
                  height={300}
                  type={chartInfo.metric.type as 'line' | 'bar'}
                  color={chartInfo.metric.color}
                  title={chartInfo.metric.label}
                  xLabel="Period"
                  yLabel={chartInfo.metric.label}
                />
              );
            })}
          </div>
